...
const wishList = [
  {
    text: "Chúc Mi một mùa Trung Thu đoàn viên, tràn ngập niềm vui và hạnh phúc!",
    img: "./assets/1.png",
  },
  {
    text: "Cầu chúc cho mọi nguyện ước của Mi đêm nay sẽ trở thành hiện thực.",
    img: "./assets/2.png",
  },
  {
    text: "Trăng tròn ấm áp, chúc tình Mi và tình yêu của chúng ta mãi bền chặt.",
    img: "./assets/3.png",
  },
  {
    text: "Chúc Mi luôn giữ được tâm hồn trong trẻo, yêu đời như ánh trăng rằm.",
    img: "./assets/1.png",
  },
  {
  ...
  {
    text: "Chúc riêng Mi một đêm trăng thật lãng mạn và ngọt ngào.",
    img: "./assets/3.png",
  },
  {
    text: "Yêu Mi thúi.",
...
function onPointerUp(event) {
  ...
  if (intersects.length > 0) {
    ...
    if (!window.availableWishes || window.availableWishes.length === 0) {
      window.availableWishes = Array.from({ length: wishList.length }, (_, i) => i);
    }

    const randomIndex = Math.floor(Math.random() * window.availableWishes.length);
    const wishIndex = window.availableWishes.splice(randomIndex, 1)[0];
    const randomWish = wishList[wishIndex];
